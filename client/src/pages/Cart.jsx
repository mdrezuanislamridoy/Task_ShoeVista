    ...
    const fetchData = async () => {
      try {
        const res = await axios.get(
          `${import.meta.env.VITE_BASE_URL}/api/products/${cartList.join(",")}`,
        );
        if (isMounted) {
          if (res.data.message) {
            setProducts([]);
          ...
          <div className="xs:w-[95vw] sm:[90vw] md:w-5/6 mx-auto text-center sm:flex justify-between">
            <div className="flex flex-col xs:text-xs md:text-sm p-1 sm:w-1/2 border-1">
              {products.map((product) => {
                const item = cartItems.find((e) => e.id === product._id) || {
                  count: 1,
                };
                return (
                  <div
                  ...
                  >
                    <p
                      className="xs:text-xs sm:text-sm cursor-pointer hover:scale-110 absolute xs:top-0 xs:right-0"
                    ...
                    <Link
                      ...
                      className="xs:w-1/3 xs:h-2/3 h-full m-auto flex justify-center items-center  sm:mr-2 md:mr-0"
                    >
                      <img
                        src={product.img}
                        alt={product.title}
                    ...
                    <div className="xs:w-2/3 flex flex-col justify-start items-start gap-2 text-sm font-medium">
                      ...
                      <p className="xs:text-xs md:text-sm gap-2 font-medium text-gray-500">
                        ...
                        <button
                          onClick={() =>
                            decCount(product._id, product.sellPrice)
                          }
                        ...
                        <button
                          onClick={() =>
                            incCount(product._id, product.sellPrice)
                          }
                      ...
                      <div className="flex justify-between items-start flex-col w-full">
                        ...
                        {item.subTotal !== product.sellPrice ? (
                          <p className="xs:text-xs md:text-sm text-gray-500 justify-between">
                            Subtotal:{" "}
                            {`\u20B9 ${new Intl.NumberFormat("en-IN").format(item.subTotal)}`}
                          </p>
                        ) : (
                          ""
                        )}
            ...
            <div className="px-4 py-3 bg-gray-50 h-full w-1/2 md:mx-4 border-1 xs:w-full sm:w-1/2 xs:m-0 ">
              ...
              <div className="flex justify-between mb-2">
                ...
                <span>
                  -{" "}
                  {`\u20B9 ${new Intl.NumberFormat("en-IN").format(discount)}`}
                </span>
              </div>
              <div className="flex justify-between mb-2">
